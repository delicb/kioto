...
func testURLMiddleware(t *testing.T, data []testData, factory func(r *http.Request, input testData) cliware.Middleware) {
	for _, d := range data {
		...
		if _, err := factory(req, d).Exec(handler).Handle(req); err != nil {
...
func TestParam(t *testing.T) {
	for _, data := range []struct {
		...
		handler := createHandler()
		if _, err := url.Param(data.ParamKey, data.ParamValue).Exec(handler).Handle(req); err != nil {
			t.Error("Unexpected error:", err)
		}
...
func TestParams(t *testing.T) {
	for _, data := range []struct {
		...
		req.URL.Path = data.InitialPath
		handler := createHandler()
		if _, err := url.Params(data.Params).Exec(handler).Handle(req); err != nil {
			t.Error("Unexpected error: ", err)
		}
