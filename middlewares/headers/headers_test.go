...
func TestMethod(t *testing.T) {
	for _, method := range []string{
		...
		if _, err := m.Exec(handler).Handle(req); err != nil {
...
func TestFromContext_HeaderList(t *testing.T) {
	for _, data := range []struct {
		...
		if hdrs, ok := data.Value.([]headers.Header); ok {
			if len(hdrs) == 1 {
				ctx = context.WithValue(context.Background(), data.Key, hdrs[0])
			} else {
				ctx = context.WithValue(context.Background(), data.Key, hdrs)
			}
		} else {
...
func TestReadHeaderString(t *testing.T) {
	var got string
	err := testReadHeaderValue(t, "foo", &got)
	if err != nil {
...
func TestReadHeaderInt(t *testing.T) {
	var got int
	err := testReadHeaderValue(t, "42", &got)
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	if got != 42 {
...
func testReadHeaderValue(t *testing.T, toSet string, destination interface{}) error {
	t.Helper()
	headerName := "X-custom-header-name"
	req := cliware.EmptyRequest()
	handler := func(req *http.Request) (*http.Response, error) {
		...
		r.Header.Set(headerName, toSet)
		return r, nil
	}
	_, err := headers.ReadValue(headerName, destination).Exec(cliware.HandlerFunc(handler)).Handle(req)
	return err
}
